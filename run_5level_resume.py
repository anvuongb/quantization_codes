...
if __name__ == "__main__":
    ...
    # with open("data_5lv_exhaustive_data_N19.pickle", "rb") as f:
        # data = pickle.load(f)
    current_best = 1.2653921960614958
    current_best_thres = (-8.,   -4.32, -2.72,  0.,    2.88,  8.  )
    ...
    for idx_h1, h1 in tqdm.tqdm(enumerate(search_thresh[:-3])):
        ...
        for idx_h2, h2 in enumerate(search_thresh[idx_h1+1:-2]):
            for idx_h3, h3 in enumerate(search_thresh[idx_h2+1:-1]):
                for idx_h4, h4 in enumerate(search_thresh[idx_h3+1:]):
                    ...
                    tmp_best = calculate_I(Px, Q, M, Phi, [S[0], h1, h2, h3, h4, S[-1]])
                    if tmp_best > current_best:
                        current_best = tmp_best
                        current_best_thres = (h1, h2, h3, h4)
        stop = time.time()
        ...
        # if (idx_h1+1)%10 == 0:
            # with open("data_5lv_exhaustive_data_N{}.pickle".format(N), "wb") as f:
                # pickle.dump(data, f)
    with open(log_fn, "a") as f:
